...
class UniqueSnCache(object):
    ...
    def get_sn(self, SubType=""):
        if self.sn_now < 0 or self.sn_now + 1 > self.sn_max:
            ...
            result = self.sn_collection.find_one_and_update(query, update=update, return_document=pymongo.ReturnDocument.BEFORE ,upsert=False)
            ...
            if result != None:
                ...
                result = self.sn_collection.find_one_and_update(query, update=update, return_document=pymongo.ReturnDocument.AFTER, upsert=True)
    ...
    def get_sn_seq(self, nAmount, SubType=""):
        ...
        if result is not None:
            ...
            result = self.sn_collection.find_one_and_update(query, update=updInsert, return_document=pymongo.ReturnDocument.AFTER, upsert=True)
            if result is None:
                raise Exception("get_sn_seq failed")
            result = self.sn_collection.find_and_modify(query, update=update, return_document=pymongo.ReturnDocument.BEFORE, upsert=False)
            sn_head = result[self.sn_field]
            #print 'get_sn ', self.sn_collection.name, 'result3: ', result
        r = list(range(sn_head, sn_head + nAmount))
        assert len(r) == nAmount
        return r
