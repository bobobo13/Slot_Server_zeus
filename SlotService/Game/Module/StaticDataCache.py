...
class _StaticDataCache(object):
    ...
    def wrap(self, _func, *args, **kwargs):
        if six.PY2:
            ...
            fargs, fvarargs, fkeywords, fdefaults, fkwonlyargs, fkwonlydefaults, fannotations = inspect.getfullargspec(_func)
        t = []
        for i in range(len(fargs)):
            ...
            if i < len(args):
                ...
                raise TypeError("{}() takes {} {} arguments ({} given)".format(_func.__name__, "exactly" if (fdefaults is None and fvarargs is None and fkeywords is None) else "at least", len(fargs) if fdefaults is None else len(fargs)-len(fdefaults), len(args)+len(kwargs)))
            t.append((k, v))
        t2 = tuple(t)
