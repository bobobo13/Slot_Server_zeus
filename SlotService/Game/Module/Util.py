...
def ReadConfigDict(configPath, section=None):
    ConfigObj = configparser.RawConfigParser()
    ConfigObj.read(configPath)
    if section is None:
        return {s: dict(ConfigObj.items(s)) for s in ConfigObj.sections()}
...
def Convert(data):
    if isinstance(data, str):  # Python 3 没有 basestring
        return str(data)
    elif isinstance(data, collections.abc.Mapping):  # Python 3 用 collections.abc.Mapping
        return {Convert(k): Convert(v) for k, v in data.items()}  # 用 items() 代替 iteritems()
    elif isinstance(data, collections.abc.Iterable) and not isinstance(data, (str, bytes)):  # 处理可迭代对象，排除字符串和字节
