...
class BaseServer(Greenlet):
    def __init__(self, code_name='', version='dev', global_code_name=None, http_response_handler=None, passTime=60):
        ...
        self.logger.info("%s.%s ArkEngine version:%s" % (self.__class__.__name__, get_function_name(), engine_version))
    ...
    def getPath(self, function=None):
        ...
        if _platform.startswith("linux"):
            if distro.id() == 'redhat' or distro.id() == 'centos':
    ...
    def init(self, path='./', configPath='config/dev/server.cfg', version='dev'):
        try:
            ...
            raise Exception("Server:start Exception : " + str(traceback.format_exc()))
