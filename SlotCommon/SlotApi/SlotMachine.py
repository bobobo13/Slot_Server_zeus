...
class SlotMachine:
    ...
    def __init__(self, Logger, DataSource=None, CodeName="", **kwargs):
        ...
        self.calculator_cls = importlib.import_module(".Game.Slot.{0}.{0}".format(self.code_name),
                                                      package=self.code_name).__getattribute__(self.code_name)
    ...
    def start_game(self, ark_id, game_name, fs_setting, gn_data, platform_data, game_data):
        ...
        #  取得玩家遊戲狀態
        ret_game_state, bet_value = self._get_ret_game_state(game_setting, calculator, game_state, bet_list, slot_type,
                                                             lineways_amount)
        ...
        if 'MaxOdds' in game_info:
            ret['MaxOdds'] = game_info['MaxOdds']

        self.logger.debug('[SlotMachine] start_game: ret_game_state={}'.format(ret_game_state))
        self.logger.debug(
            '[SlotMachine] start_game: ark_id={}, game_name={}, code_name:{}, fs_setting={}, gn_data={}, platform_data={}, game_data:{}'.format(
                ark_id, game_name, self.code_name, fs_setting, gn_data, platform_data, game_data))
    ...
    def GetChanceKey(self, gn_data, platfrom_data, game_data):
        if platfrom_data is None:
            return None
        gameRatio = platfrom_data.get("ChanceKey")
    ...
    def spin(self, ark_id, game_name, fs_setting, gn_data, platform_data, game_data):
        ...
        self.logger.debug(
            '[spin] spin: dev_mode={}, EnableTestMode={}'.format(dev_mode, EnableTestMode))
        ...
        if game_state.is_special_game:
            self.logger.error(
                '[SlotMachine] spin: user in bonus/fever! user_id={}, game_name={}, code_name:{}, fs_setting:{}, gn_data={}, platform_data={}, game_data={}'.format(
                    ark_id, game_name, self.code_name, fs_setting, gn_data, platform_data, game_data))
            print("spin錯誤！目前在特殊遊戲中: {}".format(game_state.all_sg_id))
        ...
        if bet_value not in bet_list:
            self.logger.error(
                '[SlotMachine] spin: user bet id error! user_id={}, game_name={}, code_name:{}, fs_setting:{}, gn_data={}, platform_data={}, game_data={}'.format(
                    ark_id, game_name, self.code_name, fs_setting, gn_data, platform_data, game_data))
        ...
        prob_id, game_info = self._get_game_info(game_name, assignProbId=prob_id)
        spin_result, origin_total_win, extra_info = calculator.spin_flow(bet_value, bet_lines, game_state, game_info,
                                                                         dev_mode=dev_mode, user_id=ark_id,
                                                                         extra_bet=extra_bet)
        # self.logger.debug(
        #     '[SlotMachine] spin: extra_info={}, game_info={}, use_remote_game_state:{}'.format(extra_info, game_info, use_remote_game_state))

        if extra_info is not None:
            game_state.update_extra_info(extra_info)

        spin_result, game_state, jackpot_win, winJpList, jp_system, jackpot_con_data = self._get_jp_result(ark_id,
                                                                                                           bet_value,
                                                                                                           bet_lines,
                                                                                                           spin_result,
                                                                                                           game_state,
                                                                                                           extra_bet,
                                                                                                           calculator.JackpotFlow,
                                                                                                           self.InGameJpMgr,
                                                                                                           game_info,
                                                                                                           ratio=extra_bet_ratio,
                                                                                                           isJpWinIndependent=EnableJpWinIndependent)

        main_total_win = game_state.update_by_spin_result(spin_result, bet_value, bet_lines,
                                                          jp_win=jackpot_win if EnableJpWinIndependent else 0,
                                                          probId=prob_id)
        ...
        ret = self._get_spin_ret(spin_result, game_state, main_total_win, win_type, slot_type, current_script,
                                 bet_value, bet_lines, None, jackpot_win)

        HistoryDetail = calculator.build_result_log(None, spin_result, jp_win=0, extra_bet=extra_bet)

        # print(f"game_state_data End={game_state}")
        self.logger.info(
            '[SlotMachine] spin: ark_id={}, game_name={}, code_name:{}, fs_setting:{}, gn_data={}, platform_data={}, game_data={}'.format(
                ark_id, game_name, self.code_name, fs_setting, gn_data, platform_data, game_data))
        self.logger.debug(
            '[SlotMachine] spin: ret={}, user_game_state={}, history_detail={}'.format(ret, new_user_game_state,
                                                                                       HistoryDetail))
        return self._Result(result=STATUS_CODE_OK, data=ret, user_game_state=new_user_game_state,
    ...
    def next_fever(self, ark_id, game_name, fs_setting, gn_data, platform_data, game_data, **kwargs):
        ...
        """
        # 平台指定功能開關
        EnableTestMode = fs_setting.get('EnableTestMode', True)  # dev 模式
        EnableJpWinIndependent = fs_setting.get('EnableJpWinIndependent', False)
        ...
        use_remote_game_state = game_data.get('use_remote_game_state', False)
        game_state_data = game_data.get('game_state_data', None)
        client_action_data = dict(kwargs['client_action'])
        client_action_data['client_sg_id'] = special_game_id

        # 如果有傳 game_state_data ，則使用傳入的 game_state_data，否則從資料庫取得
        if not use_remote_game_state:
            game_state_data = self._slotDao.GetGameState(ark_id)

        # print(f"game_state_data ={game_state_data}")
        game_state = PlayerGameState(game_state_data)
        game_state.update_extra_bet(extra_bet)  # 將extra_bet資訊存入game_state
        ...
        if not game_state.is_special_game:
            ...
            print("next_fever錯誤！ {} 目前沒有特殊遊戲".format(ark_id))
            return self._Result(result=STATUS_CODE_ERR_FEVERGAME_STATE_EXCEPTION)

        # sg_id 對不起來
        if special_game_id != game_state.current_sg_id:
            self.logger.error(
                "[next_fever] sg_id is not match, {} != {}".format(special_game_id, game_state.current_sg_id))
            return self._Result(result=STATUS_CODE_ERR_FEVERGAME_STATE_EXCEPTION)
        ...
        # 進入 next_fever_flow
        fever_result, game_state = calculator.next_fever_flow(client_action_data, game_state, game_info,
                                                              dev_mode=dev_mode, user_id=ark_id, extra_bet=extra_bet)
        if fever_result.has_error:
            self.logger.error(
                '[SlotMachine][next_fever] fever result error={}. ark_id={}, game_name={}, code_name:{}, fs_setting:{}, gn_data={}, platform_data={}, game_data={}'.format(
                    fever_result.error, ark_id, game_name, self.code_name, fs_setting, gn_data, platform_data,
                    game_data))
            return self._Result(result=STATUS_CODE_ERR_FEVERGAME_STATE_EXCEPTION)
        self.logger.debug(
            '[SlotMachine][next_fever] fever_result={}, game_info={}, use_remote_game_state:{}'.format(fever_result,
                                                                                                       game_info,
                                                                                                       use_remote_game_state))

        # print("- fever_result={}".format(fever_result))
        ...
        game_state.update_last_fever_reels(special_game_id,
                                           *calculator.custom_last_fever_reels(fever_result))  # 更新 recovery 盤面
        ...
        ret['win_type'] = calculator.get_fever_win_type(original_bet, fever_result.win_amount, game_state, fever_result,
                                                        win_type_info)
        # print("- win_type_info={}, win_type={}, fever_result.win_amount:{}, original_bet:{}".format(win_type_info, ret['win_type'], fever_result.win_amount, original_bet))

        # 安全检查 current_special_game_data 是否为 None
        if now_state.current_special_game_data:
            ret['total_times'] = now_state.current_special_game_data['current_script'].get('total_times', 0)
            ret['current_times'] = now_state.current_special_game_data['current_script'].get('current_times', 0)
        else:
            # 处理 current_special_game_data 为 None 的情况
            ret['total_times'] = 0
            ret['current_times'] = 0
        ...
        # extra info
        if hasattr(fever_result, 'extra_data'):
            ret['extra_info'] = fever_result.extra_data

        # self.logger.debug(
        #     '[SlotMachine][next_fever] now_state={}'.format(now_state))

        # history_detail 實作
        history_detail = calculator.build_fever_result_log(None, fever_result, now_state)

        self.logger.info(
            '[SlotMachine][next_fever] ark_id={}, game_name={}, code_name:{}, fs_setting:{}, gn_data={}, platform_data={}, game_data={}'.format(
                ark_id, game_name, self.code_name, fs_setting, gn_data, platform_data, game_data))
        self.logger.debug(
            '[SlotMachine][next_fever] ret={}, user_game_state={}, history_detail={}'.format(ret, new_user_game_state,
                                                                                             history_detail))
        return self._Result(result=STATUS_CODE_OK, data=ret, user_game_state=new_user_game_state,
    ...
    def _get_bet_info(self, gameSetting, platform_data, game_state):
        ...
        # 如果沒有平台押注資訊，則用遊戲本身的
        if (len(platform_bet_list) == 0):
            inter_bet_list = game_bet_list
        else:
    ...
    def _get_play_game_info(self, gameSetting, calculator, game_state):
        ...
        # print("[SlotMachine][_get_play_game_info] extra_info:{}, gameSetting:{}  game_state:{}".format(
        #     extra_info, gameSetting, game_state))
    ...
    def _get_game_info(self, game_name, assignProbId=None, GameInfoList=None):
        ...
        # print("[SlotMachine][_get_game_info] gameInfo:{}, prob_id ={}  gameInfoData:{}".format(gameInfo, prob_id, gameInfoData))
    ...
    def _get_jp_result(self, ark_id, bet_value, bet_lines, spin_result, game_state, extra_bet, JackpotFlowFunc,
                       InGameJpMgr, gameInfo, enable_jp=True, ratio=1, isJpWinIndependent=True):
        jp_system = ""
        jackpot_win = 0
        winJpList = []
        ...
        if InGameJpMgr is not None:
            spin_result, game_state, jackpot_win, winJpList, jp_system, jackpot_con_data = JackpotFlowFunc(gameInfo,
                                                                                                           InGameJpMgr,
                                                                                                           ark_id,
                                                                                                           bet_value,
                                                                                                           bet_lines,
                                                                                                           ...
                                                                                                           game_state,
                                                                                                           playerData=None,
                                                                                                           isJpWinIndependent=isJpWinIndependent,
                                                                                                           extra_bet=extra_bet,
                                                                                                           ratio=ratio)
        return spin_result, game_state, jackpot_win, winJpList, jp_system, jackpot_con_data

    def _get_spin_ret(self, spin_result, game_state, main_total_win, win_type, slot_type, current_script, bet_value,
                      bet_lines, serial_id, jackpot_win, game_state_data=None):
        field_map = {
            'allways': 'current_costs',
            'lines': 'current_lines',
        }
        ...
        if game_state.current_sg_id >= 0:
            ...
            ret['game_state']['current_sg_total_times'] = game_state.current_special_game_data['current_script'].get(
                'total_times', 0)
        ...
        # print(spin_result)
        if hasattr(spin_result, 'hit_max_win'):
            ret['hit_max_win'] = spin_result.hit_max_win
